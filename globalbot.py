...
bot = commands.Bot(command_prefix='jg.')
...
async def addGlobal(ctx):
    if ctx.author.guild_permissions.administrator:
        if not guild_exists(ctx.guild.id):
            ...
            with open('servers.json', 'w') as f:
                json.dump(servers, f, indent=4)
            embed = discord.Embed(title="**Willkommen im GlobalChat von Jugendstelle**",
...
async def on_message(message):
    if message.author.bot:
        return
    if not message.content.startswith('jgd'):
...
async def sendAll(message: Message):
    ...
    links = '[Jugendstelle Discord](https://discord.gg/3KbEsy7E2A) ║ '
...
bot.run('OTEyNDk2NDcyNTQzNTk2NjQ1.YZwykA.3iGbpR6kPTvLyLQYDIJ5MDaFjWU')
